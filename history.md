...
- Sensor protocol lives in [src/lsm6ds3.rs](src/lsm6ds3.rs): a board-agnostic `Lsm6ds3` driver wrapping a `Twim<'static>` that owns the register-level LSM6DS3TR-C protocol (`probe`/`init`/`configure`/`read_reg`/`write_reg`/`read_sample`) and remembers the responding I2C address internally. It exposes `ImuIoError::{Bus, Timeout, NotDetected}` — `NotDetected` is returned immediately (no bus access, no wasted timeout) when no address has been probed yet.
- [src/main_imu.rs](src/main_imu.rs) is now thin task/channel glue, following the same pattern as `main_leds.rs`: a `Channel<ImuCommand>` + `ImuSender`/`sender()`, and an `imu_task` that drives an owned `Lsm6ds3` and formats UART responses. No register constants or raw I2C calls remain here.
- The IMU task replies directly to UART via `main_uart::UartTxSender`/`tx_sender()`, rather than routing responses back through `cli_task` — this keeps `cli_task` a pure dispatcher and lets the IMU task own all response formatting.
- `src/bsp.rs` fully constructs the ready-to-use `Twim` inside `Board::init` (now `async`), owning the `TWISPI1` interrupt binding and the `imu_power` rail settle delay itself — mirroring how it already fully builds `Leds` — so `main.rs` just hands `board.twim` straight to `main_imu::spawn` instead of wiring pins/interrupts itself.
...
- **I2C address**: probe `0x6A` first, then fall back to `0x6B` (`IMU_ADDR_PRIMARY`/`IMU_ADDR_SECONDARY`), and remember whichever address responded for subsequent `imu-r`/`imu-w` calls in the `Lsm6ds3` driver's own state (`addr: Option<u8>`).
